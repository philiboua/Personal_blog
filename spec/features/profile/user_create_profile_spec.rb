...
RSpec.feature "Blogger creates new profile" do 
  ...
  given(:profile) { create(:profile, user_id: registered_user.id )}
  ...
  context "when finish registration succesfully" do 
    ...
    scenario "new profile is saved successfully" do
      ...
      then_the_user_is_redirected_to_new_post_page 
    end
  ...
  def then_the_user_is_redirected_to_new_post_page 
    expect( page.current_path ).to eq( profile_path(Profile.last.id) )
  end
